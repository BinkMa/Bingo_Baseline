...
// LAST LEVEL CACHE
#define LLC_SET NUM_CPUS * 1
#define LLC_WAY 64143
...
public:
    ...
    void handle_prefech(double long &cnt_acc, double long &cnt_miss,uint64_t pf_addr,uint64_t ip);
    int check_hit(uint64_t addr),
    ...
    void handle_read(fstream &fin), handle_prefetch();
    ...
    void add_mshr(PACKET *packet),
            ...
            fill_cache(uint32_t set, uint32_t way, uint64_t addr),
    ...
    uint64_t llc_prefetcher_operate_(uint32_t cpu, uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type);
    uint64_t llc_prefetcher_operate(uint32_t cpu, uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type);

    uint32_t get_set(uint64_t address),
            get_way(uint64_t address, uint32_t set),
            find_victim(uint32_t cpu,  uint32_t set, const BLOCK *current_set, uint64_t ip,
                        uint64_t full_addr, uint32_t type),
            llc_find_victim(uint32_t cpu,  uint32_t set, const BLOCK *current_set, uint64_t ip,
                            uint64_t full_addr, uint32_t type),
            lru_victim(uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t ip,
                       uint64_t full_addr, uint32_t type);
};
